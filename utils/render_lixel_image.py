...
def render_point_cloud_quaternion(point_cloud_positions, point_cloud_rgb, proj_matrix, quaternion, tvec, image_size=(800, 800)):
    """
    ...
    rot_mtx = R.from_quat(quaternion).as_matrix().astype(np.float32)

    inv_rot_mtx = rot_mtx.T
    inv_t = -inv_rot_mtx @ tvec
...
def render_point_cloud_fisheye_quaternion(point_cloud_positions, point_cloud_rgb, proj_matrix, dist_coeffs, quaternion, tvec, image_size=(800, 800)):
    """
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print(f"Usage: {sys.argv[0]} lixel_traj_filename cam_idx")
        sys.exit(-1)
    ...
    camera_found = False
    project_dirname = os.path.dirname(lixel_traj_filename)
    ...
    with open(lixel_traj_filename, "r") as lixel_traj_file:
        ...
        for line in lixel_traj_file:
            ...
            image_path = os.path.join(project_dirname, "images", image_path)

            camera_name = os.path.basename(os.path.dirname(image_path))

            if cur_cam_idx == cam_idx:
                camera_found = True
                break
    ...
    if not camera_found:
        raise Exception("Unable to find camera", cam_idx)
    
    # Read in the point cloud
    ply_filename = os.path.join(project_dirname, "point_cloud.ply")

    ply_data = PlyData.read(ply_filename)
    vertices = ply_data["vertex"]
    num_vertices = len(vertices)

    point_cloud_positions = np.empty((num_vertices, 3), dtype=np.float32)
    point_cloud_positions[:, 0] = vertices["x"]
    point_cloud_positions[:, 1] = vertices["y"]
    point_cloud_positions[:, 2] = vertices["z"]

    point_cloud_rgb = np.empty((num_vertices, 3), dtype=np.uint8)
    point_cloud_rgb[:, 0] = vertices["red"]
    point_cloud_rgb[:, 1] = vertices["green"]
    point_cloud_rgb[:, 2] = vertices["blue"]

    # Extract every Nth item
    POINT_CLOUD_SAMPLE_STEP = 20
    point_cloud_positions = point_cloud_positions[::POINT_CLOUD_SAMPLE_STEP, :]
    point_cloud_rgb = point_cloud_rgb[::POINT_CLOUD_SAMPLE_STEP, :]
    ...
    def update_render():
        ...
        f = step2f(cv2.getTrackbarPos("f", "Fisheye Image"))
        cx = step2c(cv2.getTrackbarPos("cx", "Fisheye Image"))
        cy = step2c(cv2.getTrackbarPos("cy", "Fisheye Image"))
        k1 = step2k(cv2.getTrackbarPos("k1", "Fisheye Image"))
        k2 = step2k(cv2.getTrackbarPos("k2", "Fisheye Image"))
        k3 = step2k(cv2.getTrackbarPos("k3", "Fisheye Image"))
        k4 = step2k(cv2.getTrackbarPos("k4", "Fisheye Image"))


        if camera_name == "camera_0":
            fx = CAMERA_0_FX
            fy = CAMERA_0_FY
            cx = CAMERA_0_C_OFFSET_X
            cy = CAMERA_0_C_OFFSET_Y
            dist_coeffs = CAMERA_0_DIST
        elif camera_name == "camera_1":
            fx = CAMERA_1_FX
            fy = CAMERA_1_FY
            cx = CAMERA_1_C_OFFSET_X
            cy = CAMERA_1_C_OFFSET_Y
            dist_coeffs = CAMERA_1_DIST
        else:
        ...
        rot_quat = np.array([q_x, q_y, q_z, q_w], dtype=np.float32)
        tvec = np.array([t_x, t_y, t_z], dtype=np.float32)

        print(rot_quat)
        print(tvec)

        image = render_point_cloud_fisheye_quaternion(point_cloud_positions, point_cloud_rgb, proj_mtx, dist_coeffs, rot_quat, tvec, (IMAGE_WIDTH, IMAGE_HEIGHT))
        ...
        rectified_image, K_new = rectify_fisheye_image(orig_image, proj_mtx, dist_coeffs, (IMAGE_WIDTH, IMAGE_HEIGHT), balance=0.0, fov_scale=0.5)
        rectified_point_image = render_point_cloud_quaternion(point_cloud_positions, point_cloud_rgb, K_new, rot_quat, tvec, (IMAGE_WIDTH, IMAGE_HEIGHT))
